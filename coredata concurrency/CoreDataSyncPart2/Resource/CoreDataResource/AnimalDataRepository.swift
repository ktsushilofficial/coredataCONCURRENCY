...
struct AnimalDataRepository : AnimalCoreDataRepository {
    ...
    func insertAnimalRecords2(records: Array<Animal>) -> Bool {
        ...
        debugPrint("AnimalDataRepository: Insert record operation is starting")
         ...
         let privatecont = NSManagedObjectContext(concurrencyType: .privateQueueConcurrencyType)
         privatecont.parent = PersistentStorage.shared.persistentContainer.viewContext
         privatecont.perform {
             records.forEach { animalRecord in
                 let cdAnimal = CDAnimal(context: privatecont)
                 cdAnimal.id = UUID()
                 cdAnimal.name = animalRecord.name
                 cdAnimal.image = animalRecord.image
             }
             do
             {
                 try privatecont.save()
                 PersistentStorage.shared.persistentContainer.viewContext.performAndWait {
                     do
                     {
                         try PersistentStorage.shared.persistentContainer.viewContext.save()
                     }
                     catch{
